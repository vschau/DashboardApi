...
namespace DashboardApi.Helpers
{
    public class HelperMethods
    {
        ...
        public static Customer GetRandomCustomer(DashboardContext ctx)
        {
            var randomId = _rand.Next(1, ctx.Customers.Count());
            return ctx.Customers.First(c => c.Id == randomId);
        }

        public static int GetRandomCustomerId(DashboardContext ctx)
        {
            return _rand.Next(1, ctx.Customers.Count());
        }
